      ...
      if UpperCase(Sourcepath)<>UpperCase(DestPath) then
        begin
          if (crc<>'') and (FileExists(destpath)) then
            begin
              GetNesCrc(DestPath,newcrc,dummy,garbage);
              if newcrc=crc then
                begin
                  flagrom:=false;
                  dp:=dir_dupes;
...
begin
  ...
  if (NesCL.docsum=true) and (show=true) then
    begin
      GetNesCrc(Name,csum,prgcsum,garbage,nes.prgbank);
    ...
    begin
      ...
      if (dbpos>0) and (NesCL.dbase=false) then
        begin
          ...
          if NesCL.mthe=true then result:=movethe(result);
          ...
          if (namematch=false) and (NesCL.rname=false) then
            begin
              ...
              if NesCL.mthe=false then
                if LowerCase(copy(name,1,3))='the' then
                   begin rtmp:=movethe(rtmp); rtmp2:=movethe(rtmp2); end;
              if name=LowerCase(name) then
                begin rtmp:=LowerCase(rtmp); rtmp2:=LowerCase(rtmp2); end;
              if (rtmp=name) or (rtmp2=name) then namematch:=true else
                if (SpaceCvt(rtmp,true)=name) or (SpaceCvt(rtmp2,true)=name) then namematch:=true else
                  if (SpaceCvt(rtmp,false)=name) or (SpaceCvt(rtmp2,false)=name) then namematch:=true else
                    begin
                      rtmp:=result+nes.getcountry;
                      if rtmp[length(rtmp)]='.' then rtmp:=rtmp+'nes' else rtmp:=rtmp+'.nes';
                      rtmp2:=ralt+nes.getcountry;
                      if rtmp2[length(rtmp2)]='.' then rtmp2:=rtmp2+'nes' else rtmp2:=rtmp2+'.nes';
                      if NesCL.mthe=false then
                        if LowerCase(copy(name,1,3))='the' then
                          begin rtmp:=movethe(rtmp); rtmp2:=movethe(rtmp2); end;
